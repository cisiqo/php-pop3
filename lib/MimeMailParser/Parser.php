...
use MimeMailParser\Tnef\TnefAttachment,
	MimeMailParser\Exception\RuntimeException;
...
class Parser
{
		...
		if (in_array($type, array_keys($mime_types))) {
			foreach ($this->parts as $part) {
				...
				if ($this->getPartContentType($part) == $mime_types[$type]) {
					...
					if(array_key_exists('content-charset', $part)){
						$body = mb_convert_encoding($body, "UTF-8", $part['content-charset']);
					} else if (array_key_exists('charset', $part)){
	...
	{
		$attachments = array();
		$dispositions = array("attachment", "inline", "attache-inline");
		foreach ($this->parts as $part) {
			$type = $this->getPartContentType($part);
			$disposition = $this->getPartContentDisposition($part);
			if (in_array($disposition, $dispositions)) {
				if($disposition != "attache-inline"){
			...
			if ($type == 'application/ms-tnef' || $type =='application/vnd.ms-tnef') {
				$tnefAttachment = &new TnefAttachment(false);
				foreach ($attachments as $key => $val) {
					$result = $tnefAttachment->decodeTnef($val->getContent());
					$tnef_files = &$tnefAttachment->getFilesNested();
					foreach ($tnef_files as $file) {
						if (!empty($file->content)) {
							$temp_fp = tmpfile();
							$attachment = $this->decode($file->content,'');
							fwrite($temp_fp, $attachment, strlen($attachment));
							$contentType = $file->type;
							$name = $file->getName();
							if(array_key_exists('content-charset', $part)){
								$fname = mb_convert_encoding($name, "UTF-8", $part['content-charset']);
							} else if (array_key_exists('charset', $part)){
								if ($part['charset'] != "utf-8" && $part['charset'] != "utf8") {
									$fname = mb_convert_encoding($name, "UTF-8", "gbk,gb2312");
								}
							}
							$attachments[$key] = new Attachment($fname,$contentType,$temp_fp);
						}
					}
				}	
			}
	...
	private function getPartContentDisposition($part)
	{
		...
		}else if(isset($part['content-id'])){
			return "attache-inline";
		}
		return false;
	}
